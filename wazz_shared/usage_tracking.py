...
from wazz_shared.models import UserUsageStats
import os
import logging
...
def get_or_create_usage_stats(db: Session, user_id: int = None, guest_id: str = None):
    """
    ...
    Returns:
        UserUsageStats object or None if neither user_id nor guest_id is provided
    """
    if user_id:
        ...
        # Return None if neither is provided - let callers handle gracefully
        logger.warning("Usage tracking skipped: neither user_id nor guest_id provided")
        return None
...
def track_file_upload(
    ...
    # Skip tracking if no stats (neither user_id nor guest_id provided)
    if not stats:
        return
